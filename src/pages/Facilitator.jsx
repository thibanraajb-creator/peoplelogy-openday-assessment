...
export default function Facilitator() {
  ...
  const fetchData = async () => {
    ...
    setFetchError(null)
    try {
      console.log('[Facilitator] Fetching for session:', SESSION_CODE)
      ...
      console.log('[Facilitator] Org result:', orgRes)
      ...
      console.log('[Facilitator] Ind result:', indRes)

      if (orgRes.error) {
        console.error('[Facilitator] Org error:', orgRes.error)
        setFetchError(`Org: ${orgRes.error.message}`)
      }
      if (indRes.error) {
        console.error('[Facilitator] Ind error:', indRes.error)
        setFetchError(prev => prev ? prev + ` | Ind: ${indRes.error.message}` : `Ind: ${indRes.error.message}`)
      }
  ...
  const pathData = [
    { name: 'Org Only', count: orgData.length },
  ...
  const tickerItems = [...orgData, ...indData]
    ...
    .map(d => {
      ...
      return `${d.first_name} from ${d.organisation} just completed · ${d.capability_label || 'Assessment'}`
    })
  ...
  return (
    <div className="min-h-screen bg-[#0f1f35] text-white" style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      {fetchError && (
        <div className="mx-8 mt-4 bg-red-900/30 border border-red-500/30 rounded-xl px-4 py-3">
        ...
        <div className="px-8 py-6 pb-20">
          ...
          <div className="grid grid-cols-3 gap-6 mb-8">
            ...
            <div className="col-span-2 bg-white/5 border border-white/10 rounded-2xl p-5">
              ...
              {orgData.length === 0 ? (
                <p className="text-white/30 text-sm text-center pt-8">No org assessments yet</p>
              ) : (
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart data={maturityDist} layout="vertical" margin={{ top: 0, right: 30, left: 10, bottom: 0 }}>
                    ...
                    <Tooltip contentStyle={{ background: '#1B3A5C', border: 'none', borderRadius: '8px' }} labelStyle={{ color: '#fff' }} />
          ...
          <div className="grid grid-cols-2 gap-6 mb-8">
            <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
              ...
              {indData.length === 0 ? (
                <p className="text-white/30 text-sm text-center pt-8">No individual assessments yet</p>
              ) : (
                <ResponsiveContainer width="100%" height={180}>
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-5">
              ...
              {orgData.length === 0 ? (
                <p className="text-white/30 text-sm text-center pt-8">No org data yet</p>
              ) : (
                <div className="space-y-3">
                  {weakest3.map((p, i) => (
      ...
      {tickerItems.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-[#00ADA9]/90 py-3 overflow-hidden">
          <div style={{ display: 'inline-block', whiteSpace: 'nowrap', animation: 'ticker 30s linear infinite' }}>
